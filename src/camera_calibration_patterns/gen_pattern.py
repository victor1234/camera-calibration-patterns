...
#
# Copyright (c) 2000-2025, OpenCV team
#
...
class PatternMaker:
    ...
    def make_charuco_board(self):
        ...
        if dictionary["nmarkers"] < int(self.cols * self.rows / 2):
            print(
                "Error: Aruco dictionary contains less markers than it needs for chosen board. Please choose another dictionary or use smaller board than required for chosen board"
            )
            return
        ...
        if ch_ar_border < side * 0.7:
            print(
                "Marker border {} is less than 70% of ArUco pin size {}. Please increase --square_size or decrease --marker_size for stable board detection".format(
        ...
        for y in range(0, self.rows):
            for x in range(0, self.cols):
                if x % 2 == y % 2:
                    ...
                    img_mark = self._create_marker_bits(
                        markerSize_bits, dictionary["marker_" + str(marker_id)]
                    )
                    marker_id += 1
                    ...
                    square = SVG(
                        "rect",
                        x=x_pos + ch_ar_border,
                        y=y_pos + ch_ar_border,
                        width=self.aruco_marker_size,
                        height=self.aruco_marker_size,
                        fill="black",
                        stroke="none",
                    )
                    self.g.append(square)
                    for x_ in range(len(img_mark[0])):
                        for y_ in range(len(img_mark)):
                            if img_mark[y_][x_] != 0:
                                square = SVG(
                                    "rect",
                                    x=x_pos + ch_ar_border + (x_) * side,
                                    y=y_pos + ch_ar_border + (y_) * side,
                                    width=side,
                                    height=side,
                                    fill="white",
                                    stroke="white",
...
def main():
    ...
    parser.add_argument(
        "-p",
        "--marker_size",
        help="aruco markers size for ChAruco pattern (default 10.0)",
        default="10.0",
        action="store",
        dest="aruco_marker_size",
        type=float,
    )
    parser.add_argument(
        "-f",
        "--dict_file",
        help="file name of custom aruco dictionary for ChAruco pattern",
        default="DICT_ARUCO_ORIGINAL.json",
        action="store",
        dest="dict_file",
        type=str,
    )
